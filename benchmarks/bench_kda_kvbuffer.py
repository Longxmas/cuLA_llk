...
def main():
    ...
    ap.add_argument("--profile", default="",
                    choices=["", "vk", "ws", "vkkvb", "wskvb", "tpkvb", "triton", "commit", "flush"],
    ...
    if not _HAVE_TRITON:
        ...
        print(f"{'N':>4} {'T':>3} | {'Δ vk':>10} | {'Δ vkkvb':>10} | "
              f"{'Δ ws':>10} | {'Δ wskvb':>10} | {'Δ tpkvb':>10} | flag")
        for N in args.batch_sizes:
            for T in args.Ts:
                ...
                cand = [x for x in (d_vk, d_vkkvb, d_ws, d_wskvb, d_tpkvb) if x == x]
                flag = "OK" if max(cand) < args.atol else "DIFF!"
                print(f"{N:>4} {T:>3} | {d_vk:>10.2e} | {d_vkkvb:>10.2e} | "
                      f"{d_ws:>10.2e} | {d_wskvb:>10.2e} | {d_tpkvb:>10.2e} | {flag}")
...
def _timing_verify_chain(args, DSU, device):
    ...
    hdr = (f"{'N':>4} {'T':>3} {'m':>3} | {'REC_vk':>7} {'REC_ws':>7} {'REC_tri':>7} | {'KVB_vk':>7} {'KVB_ws':>7} {'KVB_tp':>7} | "
           f"{'spd_vk':>7} {'spd_ws':>7} {'spd_tp':>7} {'spd_vkbf':>8} {'spd_wsbf':>8}")
    print(hdr)
    print("-" * len(hdr))
    for r in results:
        print(f"{r['N']:>4} {r['T']:>3} {r['m']:>3} | {us(r['REC_vk']):>7} {us(r['REC_ws']):>7} {us(r['REC_tri']):>7} | "
              f"{us(r['KVB_vk']):>7} {us(r['KVB_ws']):>7} {us(r['KVB_tp']):>7} | "
              f"{rat(r['REC_vk'], r['KVB_vk']):>7} {rat(r['REC_ws'], r['KVB_ws']):>7} {rat(r['REC_ws'], r['KVB_tp']):>7} "
    ...
    hdr2 = (f"{'N':>4} {'T':>3} | {'vk_v':>6} {'ws_v':>6} {'tri_v':>6} | {'kvb_v':>6} {'wskvb_v':>7} {'tpkvb_v':>7} | "
            f"{'cmt':>5} {'flush':>6}")
    print(hdr2)
    print("-" * len(hdr2))
    for r in results:
        ...
        print(f"{r['N']:>4} {r['T']:>3} | {us(tg.get('vk_v')):>6} {us(tg.get('ws_v')):>6} {us(tg.get('tri_v')):>6} | "
              f"{us(tg.get('kvb_v')):>6} {us(tg.get('wskvb_v')):>7} {us(tg.get('tpkvb_v')):>7} | "
